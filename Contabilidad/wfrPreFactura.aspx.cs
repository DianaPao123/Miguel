...
namespace GAFWEB
{
    public partial class wfrPreFactura : Page
    {
                ...
                try
                {
                 ...
                 // ViewState["PrefacuturaDatos"] = new DatosPrefactura();
                   ...
                   this.BindDetallesToGridView();
                    ...
                    {
                        Editar(idPrefactura);
                        ViewState["idExcelCarga"] = idPrefactura; 
                    }
                    //-------------------------------fin edicion----------------------
                    else
                    { 
        ...
        */
        ...
        {
            if (lblTotal.Text != lblTotalReal.Text)
            {
                this.lblError.Text = "El total debe ser igual al total de la factura";
                return false;
        ...
        private void GuardarFactura()
        {
            ...
            if (ValidarFactura())
            {
                ...
                {
                    string xx = "OK";
                   var x= ViewState["idExcelCarga"];
                   if (x != null)
                   {
                       Int64 m = Convert.ToInt64(x);
                       var pre = new GAFTransferenciasExcel();
                       using (pre as IDisposable)
                       {
                          xx=  pre.Confirmacion(m);
                         if (xx != "OK")
                             this.lblError.Text = xx;
                       }
                   }
                    if(xx=="OK")
        ...
        private DatosPrefactura  GetFactura(string iniciales, List<Datosdetalle> detalles)
        {
            ...
            if (!string.IsNullOrEmpty(txtFechaEmision.Text))
            {
                ...
                fechaEmision = Convert.ToDateTime(fecha + " " + hora);
        ...
        */
        ...
        { 
            ...
            using (pre as IDisposable)
            {
                var fact = pre.GetTransferenciaId(id);

                var concep = pre.GetTransferenciaConceptosId(id);
                ...
                else
                {
                    ...
                    using (Empresa as IDisposable)
                    {
                      //  var em = Empresa.GetById((int)fact.IdEmpres);
                    ...
                    ddlEmpresa.SelectedValue = fact.IdEmpres.ToString();
                    if (ddlEmpresa.Items.Count > 0)
                    {
                        var cliente = new GAFClientes();
                        using (cliente as IDisposable)
                        {
                           ...
                           // var clientes = cliente.GetList(fact.IdEmpresa, string.Empty, false);
                            var clien = cliente.GetCliente((int)fact.IdCliente);
                  ...
                  //  txtConfirmacion.Text = fact.Confirmacion;
                   ...
                   // txtProyecto.Text = fact.Proyecto;
                    CultureInfo cul = CultureInfo.CreateSpecificCulture("es-MX");
                     decimal total=(decimal)fact.Total;
                    lblTotalReal.Text =total.ToString("C", cul);
                    //-------conceptos------------

                    if (concep != null)
                    { foreach (var con in concep)
                        {
                            AgregarConceptosTrasferencia(con);
                        }
                    }
        ...
        private void AgregarConceptosTrasferencia(TransferenciasExcelConceptos con)
        {
            this.txtCantidad.Text = con.Cantidad.ToString();
            txtDescripcion.Text = con.Descripcion;
            this.txtCodigo.Text = con.ClaveProdServ;
            this.txtPrecio.Text = con.ValorUnitario.ToString();
            txtIdProducto.Value = con.ID.ToString();
            ddlClaveUnidad.SelectedValue = con.ClaveUnidad;
